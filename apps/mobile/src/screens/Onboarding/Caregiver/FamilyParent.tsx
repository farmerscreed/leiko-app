...
function labelForZone(iana: string): string {
  return ZONES.find((z) => z.iana === iana)?.label ?? iana;
}
    ...
    >
      ...
      >
        ...
        >
          ...
          <Pressable
            accessibilityRole="button"
            accessibilityLabel={`Their timezone: ${labelForZone(timezone)}. Tap to change.`}
      ...
      <BottomSheet
        ...
        onDismiss={() => setZonePickerOpen(false)}
        size="tall"
        title="Choose their timezone"
        testID="family-parent-zone-sheet"
      >
        <ScrollView keyboardShouldPersistTaps="handled">
          {ZONES.map((z) => (
            <ListRow
              ...
              title={z.label}
              subtitle={z.iana}
              selected={z.iana === timezone}
              onPress={() => {
                setTimezone(z.iana);
                setZonePickerOpen(false);
              }}
              accessibilityLabel={`${z.label}${z.iana === timezone ? ', current selection' : ''}`}
              testID={`family-parent-zone-${z.iana}`}
            />
          ))}
        </ScrollView>
      </BottomSheet>

...
5. POST to `/sync` Edge Function. On success: the reading lands in `public.readings` and is dropped from the MMKV pending buffer (no on-device relational DB — see ADR-0009).

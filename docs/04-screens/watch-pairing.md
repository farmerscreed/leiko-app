...
| 3b | Watch not found → Web Bluetooth handoff (Android only) | Bottom sheet: "Pair from your parent's Android phone" with QR code that opens `https://pair.leiko.app/{url_token}` on their device |
...
2. Parent scans QR → opens `https://pair.leiko.app/{url_token}` in Chrome.
3. Single-page Next.js flow (`/leiko-pair-web` repo) requests `navigator.bluetooth.requestDevice({ filters: [{ services: ["6E40FFF0-..."] }]})`.
...
6. After first reading, web page suggests: *"Want ongoing sync? Install the Leiko app."* (links to App Store / Play Store).
...
iOS Safari does NOT support Web Bluetooth. The pairing page detects this (`navigator.bluetooth === undefined`) and routes to a Universal Link: `leiko://pair?token=...`

- If the Leiko app is installed: opens the app, restores the pairing context.
...
- "Watch found" announcement reads model + last-4-of-MAC ("Leiko Watch ending 4F2C").
...
- Web Bluetooth handoff QR generates a valid `pair.leiko.app/{url_token}` URL.

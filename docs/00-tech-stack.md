...
| File system (cache) | expo-file-system | 19.0.x (added 2026-06-05 — doctor-PDF file-share fix: download the generated report to cache so the OS sheet shares the actual FILE; the old text-URL share expired after 1 hour. Expo first-party.) |
| Native share sheet (files) | expo-sharing | 14.0.x (added 2026-06-05 alongside expo-file-system — shares local file URIs with mime type; RN's built-in Share only handles text/URL.) |
| PDF rendering (in-app preview) | react-native-pdf | 7.0.x (added 2026-06-05, founder-approved — PdfPreview screen shows the generated doctor report in-app. Requires react-native-blob-util.) |
| Blob/file transfer (peer of react-native-pdf) | react-native-blob-util | 0.24.x (peer dependency of react-native-pdf; not used directly.) |

...
- **Direction is resolved server-side at accept time**, from watch
  ...
  | Yes | Yes | **Mutual** — each follows the other's circle |
...
1. **Mutual-follow default**: when both wear watches, auto-mutual, or ask?
   (Lean: auto-mutual — they connected on purpose. Revisit if it feels
   presumptuous.)
2. **Email match**: keep the email-must-match gate (current accept
   requires it), or relax to code-only now that direction is data-driven?
   (Lean: keep email match — it's a cheap anti-mistake guard.)
3. **Back-compat window length** for the old four functions.

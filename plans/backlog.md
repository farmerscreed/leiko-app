...
- **BottomSheet polish backlog** — three deferred behaviours from Sprint 1. (a) ~~`'full'` sizing variant~~ — **RESOLVED 2026-05-07 in Sprint 1.5 / Task 8.** 95%-height variant landed alongside the glass-material + spring-rise migration. (b) drag-UP-to-expand to 90/95% on `default`/`tall` sizes — STILL DEFERRED. (c) `'swipeable'` Card variant (needs gesture-handler usage on Card) — STILL DEFERRED. Remaining items belong in a polish PR when the first real BottomSheet consumer (Inline Explainer in Sprint 11+) reveals which are actually needed. *(From Sprint 1, 2026-05-06; partial resolution 2026-05-07.)*
...
- **buildTheme D8-name compat shims** — Sprint 1.5 added shims in `apps/mobile/src/theme/buildTheme.ts` so unmigrated screens keep compiling: `theme.colors.brand.{accent,primarySoft}` aliases `brand.primary` / `text.secondary`; `theme.colors.border.default` aliases `border.subtle`; `theme.fontFamily.{body,display,numeric}` aliases the new `fontFamilies` family names. About 10 screens still consume these (CaregiverHome, SignUp, SignIn, OTPVerify, AccountTypeFork, TakeReadingScreen, ReadingDetailScreen, SelfBuyerHomePlaceholder, PairingScreen, several Onboarding). Each render correctly with D12 hex through the shims, so this is not a regression. Cleanup happens piecewise as each screen is touched in its respective successor sprint (CaregiverHome → 7.7; others → polish backlog or successor-screen sprints). Track as "remove the shim block in `buildTheme.ts` once the last consumer migrates." *(From Sprint 1.5, 2026-05-07.)*
- **WCAG 2.2 AA automated contrast runner** — D12 §2.6 specifies all foreground/background pair ratios; the new D12 token resolver returns the matching hex. Sprint 1.5 manually verified ratios via the spec table but did not ship an automated test. A small Jest test that walks every semantic-token pair × both modes and asserts ratios via a wcag-contrast helper would catch palette drift. Target: Sprint 17 (launch CI gates) or sooner if the palette is touched. *(From Sprint 1.5, 2026-05-07.)*

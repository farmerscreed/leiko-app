...
- **Bundle ID mismatch** — `apps/mobile/app.json` declares `bundleIdentifier`/`package` = `com.kena.app`, but the `apps/mobile/android/` native folder was generated by `create-expo-app` *before* we updated `app.json` and was scaffolded with package id `com.mobile` (derived from the workspace folder name). The Sprint 0 simulator launch installed and ran successfully as `com.mobile`. **Fix path**: run `npx expo prebuild --clean` (regenerates `android/` + `ios/` from `app.json`) as the first task of **Sprint 1**, before any custom native changes accumulate. Verify with `adb shell pm list packages | grep kena` after a fresh install. *(From Sprint 0 simulator setup, 2026-05-06.)*
